...
export default function TimelineScreen() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      stripRef.current?.scrollTo({ x: currentMonth * 72, animated: true });
      listRef.current?.scrollToIndex({ index: currentMonth, animated: true, viewPosition: 0 });
    }, 300);
  }, [fetchBuckets]);
  ...
  const handleMonthSelect = useCallback((index: number) => {
    ...
    track('timeline_month_selected', { month: MONTHS[index] });
    stripRef.current?.scrollTo({ x: index * 72, animated: true });
        ...
        {MONTHS.map((m, i) => {
          ...
          return (
            <Pressable
              ...
              style={{ width: 64, alignItems: 'center', marginHorizontal: 4, paddingVertical: 6, borderRadius: 10, backgroundColor: active ? colors.primary : colors.surface }}
            >
      ...
      {buckets.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <Animated.View entering={FadeInDown.delay(index * 50)} style={{ width: 140, marginHorizontal: 6, backgroundColor: item.monthIndex === selectedMonth ? colors.primarySurface ?? colors.surface : colors.surface, borderRadius: 14, padding: 10, minHeight: 120, borderWidth: item.monthIndex === selectedMonth ? 2 : 1, borderColor: item.monthIndex === selectedMonth ? colors.primary : colors.border }}>
              <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 13, color: item.monthIndex === selectedMonth ? colors.primary : colors.textSecondary, marginBottom: 6 }}>{MONTHS[item.monthIndex]}</Text>
